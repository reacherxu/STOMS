...
//	$("#txtNewName").val("");
	...
	$("#studyOutlay").val("");
	$("#studyOutlay_sr").val("");
	$("#studyOutlay_sr_test").val("");
	$("#studyOutlay_sr_energy").val("");
	$("#studyOutlay_sr_meeting").val("");
	$("#studyOutlay_sr_publish").val("");
	$("#studyOutlay_sr_other").val("");
	$("#studyOutlay_em").val("");
	$("#studyOutlay_em_material").val("");
	$("#studyOutlay_em_other").val("");
	$("#studyOutlay_ei").val("");
	$("#studyOutlay_ei_purchase").val("");
	$("#studyOutlay_ei_produce").val("");
	$("#studyOutlay_lr").val("");
	$("#studyOutlay_colaborate").val("");
	$("#international").val("");
	$("#international_1").val("");
	$("#international_2").val("");
	$("#service").val("");
...
//开卡国家自然科学基金  预算的处理
function uploadCompleteBudget(file) {
	try {
		/*  I want the next upload to continue automatically so I'll call startUpload here */
		if (this.getStats().files_queued > 0) {
			this.startUpload();
	...
	$("#divFileProgressContainer").html("");
	generalAjaxCallToLoadData("getBudgetToForm.action",{}, initializeAddBudget);
}
//开卡国家自然科学基金的回显表单
...
// 增加 开卡国家自然科学基金  预算的回显表单
function initializeAddBudget(data) {
	if(!data.actionStatus) {
		console.log("error!");
		return false;
	}
	...
	//先清空表单
	resetBudgetForm();
//	$("#txtDepartment").val("自然基金");
//	$("#txtManager").val(kaige["teacherName"]);
//	$("#txtProjectName").val($.trim(kaige["itemName"]));
//	$("#txtContractID").val(kaige["contractId"]);
	
	$("#txtContractID").val(kaige["contractId"]); //暂用合同id代替项目编号
	$("#txtName").val(kaige["teacherName"]); //负责人
	//Budget 部分
	$("#studyOutlay").val(kaige["studyOutlay"]);
	$("#studyOutlay_sr").val(kaige["studyOutlay_sr"]);
	$("#studyOutlay_sr_test").val(kaige["studyOutlay_sr_test"]);
	$("#studyOutlay_sr_energy").val(kaige["studyOutlay_sr_energy"]);
	$("#studyOutlay_sr_meeting").val(kaige["studyOutlay_sr_meeting"]);
	$("#studyOutlay_sr_publish").val(kaige["studyOutlay_sr_publish"]);
	$("#studyOutlay_sr_other").val(kaige["studyOutlay_sr_other"]);
	$("#studyOutlay_em").val(kaige["studyOutlay_em"]);
	$("#studyOutlay_em_material").val(kaige["studyOutlay_em_material"]);
	$("#studyOutlay_em_other").val(kaige["studyOutlay_em_other"]);
	$("#studyOutlay_ei").val(kaige["studyOutlay_ei"]);
	$("#studyOutlay_ei_purchase").val(kaige["studyOutlay_ei_purchase"]);
	$("#studyOutlay_ei_produce").val(kaige["studyOutlay_ei_produce"]);
	$("#studyOutlay_lr").val(kaige["studyOutlay_lr"]);
	$("#studyOutlay_colaborate").val(kaige["studyOutlay_colaborate"]);
	$("#international").val(kaige["international"]);
	$("#international_1").val(kaige["international_1"]);
	$("#international_2").val(kaige["international_2"]);
	$("#service").val(kaige["service"]);
	$("#management").val(kaige["management"]);
	$("#sum").val(kaige["sum"]);
}
...
//支出 入库处理
function uploadCompleteExpenditure(file) {
	try {
		/*  I want the next upload to continue automatically so I'll call startUpload here */
		if (this.getStats().files_queued > 0) {
			this.startUpload();
	...
	$("#divFileProgressContainer").html("");
	//
	generalAjaxCallToLoadData("uploadExcel.action",{}, alertSuccess);
}
//入账省基金的回显表单
