var projectLeaderLevelStatisticsListTable;
...
$(document).ready(function(){
	...
	tableTagArray.push({ "sTitle": "教师ID", "sClass": "center", "sWidth": "15%" });
	tableTagArray.push({ "sTitle": "教师姓名", "sClass": "center", "sWidth": "15%" });
	...
	projectLeaderLevelStatisticsListTable = $("#projectLeaderLevelStatisticsListTable").dataTable({
	    "aaData": tableData,
...
//项目负责人级查询
function projectLeaderLevelStatisticsQuery() {
	console.log("项目负责人级查询");
	...
	generalAjaxCallToLoadData("projectLeaderLevelStatisticsQuery.action",submitDatas,renderAllQueryResult);
...
function renderAllQueryResult(data) {
	...
	var projectLeaderLevelQueryResult = data.jsonResult;
	console.log(projectLeaderLevelQueryResult);
	
	//清空表格内的数据
	projectLeaderLevelStatisticsListTable.fnClearTable();
	
	for(var i = 0; i < projectLeaderLevelQueryResult.length; i++) {
		
		var tempItemInfo = projectLeaderLevelQueryResult[i];
		var tempRowData = new Array();
		tempRowData.push(tempItemInfo.teacherId);
		...
		//添加一行数据
		projectLeaderLevelStatisticsListTable.fnAddData( tempRowData );
	}
