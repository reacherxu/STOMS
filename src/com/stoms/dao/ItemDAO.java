...
public class ItemDAO extends HibernateDaoSupport {
	...
	public List findItemInfoByManager(int[] typepks, int isCross,
			int[] departmentpks) {
		Session session = this.getSession();
		log.debug("finding ItemInfo By Manager");
		try {
			...
			if (typepks != null && typepks.length > 0) {
				for (int i = 0; i < typepks.length; i++) {
					if (i == 0) {
						queryString += " and (model.projectType.typePk="
								+ typepks[i] + " or ";
					}
					queryString += " model.projectType.typePk=" + typepks[i]
							+ " or ";
					if (i == typepks.length - 1) {
						queryString += " model.projectType.typePk="
								+ typepks[i] + ")";
					}
				}
			}
			...
			if (departmentpks != null && departmentpks.length > 0) {
				for (int i = 0; i < departmentpks.length; i++) {
					if (i == 0) {
						queryString += " and (model.department.departmentPk="
								+ departmentpks[i] + " or ";
					}
					queryString += " model.department.departmentPk="
							+ departmentpks[i] + " or ";
	...
	public List findCardIDbyTeacherID(Object value) {
		log.debug("finding Item instance with Dim: TeacherID" + ", value: "
				+ value);
		try {
	...
	public List findSpareItemlistByManager(String departmentType, int isCross,
			int[] departmentpks) {
		Session session = this.getSession();
		...
		try {
			String queryString = "from Item as model where 1=1 ";
			// 判断departmentType
			if (!departmentType.equals("") && departmentType != null
					&& !departmentType.equals("所有")) {
				queryString += " and model.departmentType='" + departmentType
						+ "'";
			}
			// 判断是否为横向项目
	...
	public List findTeacherOulayItemlistByManager(String departmentType,
			int isCross, int[] departmentpks, String teacherName) {
		Session session = this.getSession();
		log.debug("finding SpareItemlist By Manager");
		try {
			String queryString = "from Item as model where 1=1 ";
	...
	public List findItemlistWithIsCrossAndProjectStatus(int isCross,
			Object value) {
		Session session = this.getSession();
		log.debug("finding Item instance with Dim: TeacherIDandCreatID"
				+ ", value: " + value);
	...
	public List findStatisticsItemlistByManager(int[] typepks, int isCross,
			int[] departmentpks) {
		Session session = this.getSession();
	...
	public List findByProjectStatusAndoperatorId(String operatorId) {
		Session session = this.getSession();
		try {
			String queryString = "from Item as model where model.projectStatus='11' and model.operatorId='"
					+ operatorId + "'";
