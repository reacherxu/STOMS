...
public class ItemService {
	...
	/**
	 * 通过itemId查项目是否有contractID 
	 * @param itemId
	 * @return 返回contractID
	 */
	public String acquireItemContractID(String itemId) {
		String result = null;
		
		List<Item> tmpItem = itemDAO.findByItemId(itemId);
		if(tmpItem != null && tmpItem.size() > 0) {
			Item i = tmpItem.get(0);
			result = i.getContractId();
		}
	 ...
	 * @return
	 */
	public boolean updateMapping(String itemId,String contractID) {
		
		List<Item> tmpItemList = itemDAO.findByItemId(itemId);

		if( tmpItemList.size() == 0)
			return false;
		
		Item tempItem = tmpItemList.get(0);
	...
	private Item setItemValue(Item tmpItem,String itemId, String contractID) {
		tmpItem.setItemId(itemId);
		tmpItem.setContractId(contractID);
		return tmpItem;
	}
	...
	public List acquireItemInfo(int[] typepks, int isCross, String lowYear,
		...
		if (lowYear != null && !"".equals(lowYear)) {
			for (Item item : iteminfolist) {
				...
				if (item.getTimeLower() != null
						&& item.getTimeLower().compareTo(lowYear) < 0) {
					delList1.add(item);
				}
			}
		}
		...
		if (upperYear != null && !"".equals(upperYear)) {
			for (Item item : iteminfolist) {
				...
				if (item.getTimeLower() != null
						&& item.getTimeLower().compareTo(upperYear) > 0) {
					delList2.add(item);
				}
			}
		}
	...
	// 管理员功能：单个项目信息查询（查询条件为typepks，是否横向，departmentpks，时间段）
	public List acquireItemInfo2(String itemid, String contractid,
			String typeid, String itemname, String departmentid,
			String departmenttype, String teachername, String lowYear,
			String upperYear) {
		List<Item> iteminfolist = itemDAO.findItemInfoByManager2(itemid,
				contractid, typeid, itemname, departmentid, departmenttype,
				teachername, lowYear, upperYear);
	...
	public String acquireItemInfoByItemID(String itemID) {
		...
		String[] excludes = { "teacher", "department", "projectType",
				"creatId", "creatName", "teacherId", "teacherName",
				"departmentType", "departmentName", "itemValue", "cardId",
	...
	public List acquireStatisticsItem(int[] typepks, int isCross,
		...
		statisticsItemByManagers.add(new StatisticsItemByManager("all",
				countapply));
		statisticsItemByManagers.add(new StatisticsItemByManager("31",
				countapprove));
		statisticsItemByManagers.add(new StatisticsItemByManager("310",
				counting));
		statisticsItemByManagers.add(new StatisticsItemByManager("311",
				countfinish));
	...
	public String acquireYearRangeByItemID(String itemID) {
		...
		if (itemList != null || itemList.size() > 1) {
			...
			result = tempItem.getTimeLower() + " 至  " + tempItem.getTimeUpper();
	...
	public String creatItemID(String departmentId, String typeId) {
		...
		// continue;
		// }
		// if("0205151".equals(itemString) && maxCount == 600 && itemIdCount <
		// 600){
	...
	public String findItemByTeacherId(String teacherId) {
	...
	public Item modifyItem(Long itemPK, String itemName, String contractID,
			...
			int departmentPk, String departmentId, String departmentName,
			String departmentType, String cardid, Double itemValue,
			Double remitValue, String timeLower, String timeUpper,
			int isFinished) {
	...
	/**
	 * 保存项目的创建者的工号和姓名 modified by shi 20131008
	 * 
	 * @param originalItem
	 * @return
	 */
	private Item saveCreatorIDAndName(Item originalItem) {
		...
		String currentTeacherID = (String) session.getAttribute("curr_adminID");// 此处creatorID和creatorName
																				// 改为管理员的session20131007shi
																				// 当需要教师自己录入时
																				// 可以改回教师的session
		String currentTeacherName = (String) session.getAttribute("adminName");
	...
	private Item setItemProperties(Item oldItem, String itemName,
			int itemTypePk, String itemType, String contractID, int isCross,
			String teacherName, String otherTeacher, String teacherId,
			int departmentPk, String departmentName, String departmentType,
			Double itemValue, String timeLower, String timeUpper,
			String applyYear) {
